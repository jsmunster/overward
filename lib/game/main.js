...
.defines(function(){
  ...
  MyGame = ig.Game.extend({
    ...
    init: function() {
      ...
      this.virus.exp_cost = ig.superStats.expAmount;
  ...
  GameOverScreen = ig.Game.extend({
    ...
    init: function() {
      ...
      this.score = ig.superStats.winCount*7 - ig.superStats.loseCount*9;
      this.expGained = Math.abs(this.score)/9+this.score/11.0;
      this.bandwidthGained = Math.abs(this.score)/16.0+this.score/17.0;

      console.log(this.expGained,this.bandwidthGained, ig.superStats);
      if(this.expGained>0){ig.superStats.expAmount += this.expGained;}
      if(this.bandwidthGained>0){ig.superStats.maxBandwidth += this.bandwidthGained;}
    ...
    draw: function() {
      ...
      this.font.draw(
          "Score This Round: "+this.score.toFixed(1)
          +"\nExperience Gained: "+this.expGained.toFixed(1)
          +"\nMaximum Bandwidth: "+this.bandwidthGained.toFixed(1)
          +"", 
          x, y, 
          ig.Font.ALIGN.CENTER);
